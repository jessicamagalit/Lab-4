...
cont_table_salamander <- table(salamander_data$section, salamander_data$unittype)

# Perform Chi-square test
chi_test_salamander <- chisq.test(cont_table_salamander)
print(chi_test_salamander)
```
This code filters the data for coastal giant salamanders and removes rows with missing values in the unittype/section columns. It then creates a table showing the relationship between forest type (section) and chanel unit (unittype) and performs a chi-square test to check if these variables are related. The end result shows a significant relationship p-value < 2.2e-16), meaning the two variables are likely linked.
...
```{r}
salamander_clean <- salamander_data |> 
  drop_na(weight_g)

# Visualize weight distribution
salamander_clean |> 
  ggviolin(x = "section", y = "weight_g", add = "boxplot", color = "section", 
           palette = c("#00AFBB", "#E7B800"))

# Extract weight data for each forest type
cc_weight <- salamander_clean |> filter(section == "CC") |> pull(weight_g)
og_weight <- salamander_clean |> filter(section == "OG") |> pull(weight_g)

# Check variance equality
var_test <- var.test(cc_weight, og_weight)
print(var_test)

# If variances are unequal, use Welch t-test
if (var_test$p.value < 0.05) {
  t_test_salamander <- t.test(weight_g ~ section, data = salamander_clean, var.equal = FALSE)
} else {
  t_test_salamander <- t.test(weight_g ~ section, data = salamander_clean, var.equal = TRUE)
}
print(t_test_salamander)
```
The code filters the data for coastal giant salamanders and removes any rows with missing weight values. it then visualizes the distribution of weight for each forest type (section) using a violin plot. The code checks if the variances in weight between the clearcut (CC) and old-growth (OG) forest types are equal using a variance test. The test result shows a significant differences in species biomass between the two forest types (p-value 8.629e-07), with the clear-cut forest type having a higher mean weight.
...
# Check normality with Shapiro-Wilk test (using a sample of 5000 rows)
shapiro_length <- shapiro.test(sample(trout_data$length_2_mm, min(5000, nrow(trout_data))))
shapiro_weight <- shapiro.test(sample(trout_data$weight_g, min(5000, nrow(trout_data))))
print(shapiro_length)
print(shapiro_weight)

# If data is skewed, log transform
trout_data <- trout_data |> 
  mutate(log_length = log(length_2_mm), log_weight = log(weight_g))

# Pearson correlation test on log-transformed data
cor_test_trout <- cor.test(trout_data$log_length, trout_data$log_weight)
print(cor_test_trout)

# Visualize correlation
trout_data |> 
  ggscatter(x = 'log_length', y = 'log_weight', alpha = .35, add = "loess")

```
this code checks if cutthroat trout body length and weight are normally distributed. Since they are not, both variables are log-transformed. A pearson correlation test shows a very strong positive relationship between the log-transformed length and weight (correlation = 0.997).
...
```{r}
selected_sites <- c("GTM", "DB", "PIE")
pie_sites <- pie_crab |> filter(site %in% selected_sites)

# Check for equal variance
levene_test <- leveneTest(size ~ site, data = pie_sites)
print(levene_test)

# Perform ANOVA
anova_model <- aov(size ~ site, data = pie_sites)
summary(anova_model)
...
```
The boxplot reveals the distribution of caraspace width fot crabs at three selected sites ordered by latitude (GTM, DB, and PIE). The tukey HSD test shows significant differences in caraspace width between the sites, with GTM crabs being significantly smaller than those at PIE, and DB crabs being smaller than those at PIE as well. Specifically, the differences in means between these pairs are statistically significant (p-values are all <0.001).
...
```{r}
lm_water_temp <- lm(size ~ water_temp_sd, data = pie_crab)
summary(lm_water_temp)

# Plot regression with line of best fit
pie_crab |> 
  ggscatter(x = 'water_temp_sd', y = 'size', alpha = .35, add = "reg.line")
```
The linear regression indicates that water_temp_sd does not significantly predict caraspace width (p-value = 0.528). The R-squared value is very small (0.001), suggesting a very weak relationship between these variables.
...
```{r}
cor_table <- pie_crab |> dplyr::select(latitude, air_temp_sd, water_temp_sd) |> cor()
print(cor_table)
...
summary(mlr_model)
```
latitude significantly affects carapace width (p< 2e-16), while air_temp_sd and water_temp_sd are not significant predictors (p= 0.1730 and p= 0.3254). The model explains 35.2% of the variance (R-squared = 0.3516).
